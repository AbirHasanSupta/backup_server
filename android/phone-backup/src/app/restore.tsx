...
function mapFeedItem(f: any): RemoteFile {
  const items: RemoteFile[] = (f.items || []).map((it: any) => ({
    path: it.path,
    size: it.size ?? 0,
  ...
  return {
    ...
    size: items.reduce((acc, it) => acc + (it.size || 0), 0) || f.size || 0,
